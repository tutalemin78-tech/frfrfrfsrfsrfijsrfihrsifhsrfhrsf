...
async def _start_broadcast_flow(event, uid, state):
    ...
    if folders:
        ...
        await show(event, "%s <b>Папки у аккаунта не найдены.</b>\n\n"
                          ...
                          "где состоит аккаунт.\n\n"
                          "<i>Если в Telegram папки есть, а тут их нет — выйди и открой рассылку ещё раз, "
                          "список обновится. Если и тогда пусто — аккаунт не залогинен/не видит чаты.</i>" % em("folder"),
                   kb.folders_kb([], []))
