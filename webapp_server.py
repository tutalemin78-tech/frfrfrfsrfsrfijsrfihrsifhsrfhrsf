...
async def api_folders(request):
    """Список папок первого аккаунта — для выбора в мини аппе."""
    user, body = await _auth(request)
    if not user:
        return _cors(web.json_response({"ok": False, "error": "bad_init_data"}, status=403))
    sessions = db.get_sessions(user.get("id"))
    folders = []
    if sessions:
        try:
            ...
            folders = []
    return _cors(web.json_response({"ok": True, "folders": folders}))
    ...
    if action == "pause":
        ...
        if folder_ids:
            ...
            try:
                ...
                _all = await worker.get_user_folders(_sess[0]) if _sess else []
        ...
        worker.start_broadcast_task(
            payload={"type": "text", "text": _md_to_html(text)},
