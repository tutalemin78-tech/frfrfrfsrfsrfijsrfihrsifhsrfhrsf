...
async def _fetch_folders(app):
    folders = []
    try:
        async for f in app.get_folders():
            title = getattr(f, "title", None)
            title = getattr(title, "text", title)
            folders.append({"id": f.id, "title": str(title or ("Папка %s" % f.id))})
    except Exception:
        try:
            ...
            pass
    # убираем служебную «Все чаты» (id=0) и дубли
    seen, out = set(), []
    for f in folders:
        if f["id"] in seen:
            continue
...
async def get_user_folders(session):
    app = await _open(session)
    out = []
    try:
        await app.start()
